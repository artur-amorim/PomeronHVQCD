...
double Numerov::diff(double E){
    ...
    std::function<double(double)> shiftedPot = [this, &E](double x) { return E - SolvSpec::potFunc.interp(x);};
  	...
  	matchPointIndex = std::max(4, matchPointIndex);
  	matchPointIndex = std::min(nPoints - 6, matchPointIndex);
...
void Numerov::findSpectrum(int nEigen)
{
  ...
  std::function<Range(double, double, int)> explore = [&](double from, double to, int N = 10) 
  {
    ...
    if(N > MAX_DIVISIONS) 
    {
      std::cout << "[WARN] The number of divisions " << N << " has gone beyond the limit for interval [" << from <<", " << to << "], aborting..." << std::endl;
  ...
  auto findSpecSol = [&](int index) 
  {
    ...
    while(!hasBeenComputed(index)) 
    {
      ...
      Mode m = findSol(r);
      std::cout << "new mode found " << m.index << std::endl;
      modes.push_back(m);
      ...
      if(attempts > 10) 
      {
        std::cout << "[WARN] Too many attempts while finding eigenvalue " << index << ", computation is compromised." << std::endl;
        break;
      }
    }
  };
  ...
  {  //TODO: finish this nice stuff
    ...
    { // we need to look for a missing mode
      std::cout << "Looking back for E(" << i << ")..." << std::endl;
      try 
      {
        findSpecSol(i);
      }
