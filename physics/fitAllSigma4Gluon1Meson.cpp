...
int main(int argc, char ** argv)
{
    ...
    int coeff_index;
    double k0g, k1g, k2g, k3g, k0m;
    // Gravitational proton couplings
    double kbar0g, kbar1g, kbar2g, kbar3g, kbar0m;
    //string sigma_gp_path, sigma_gg_path, sigma_pp_path;
    if(argc < 10)
    {
        cout << "Program usage: " + string(argv[0]) + " coeff_g coeff_m coeff_index k0g k1g k2g k3g k0m kbar0g kbar1g kbar2g kbar3g kbar0m " << endl;
        return 0;
    }
    else
    {
        coeff_g = stod(argv[1]); coeff_m = stod(argv[2]); coeff_index = stoi(argv[3]);
        k0g = stod(argv[4]); k1g = stod(argv[5]); k2g = stod(argv[6]); k3g = stod(argv[7]);  k0m = stod(argv[8]);
        kbar0g = stod(argv[9]); kbar1g = stod(argv[10]); kbar2g = stod(argv[11]); kbar3g = stod(argv[12]); kbar0m = stod(argv[13]);
    }
    cout << "Starting fit with values:" << endl;
    cout << "coeff_index: " << coeff_index << endl;
    cout << "coeff_g: " << coeff_g << " coeff_m: " << coeff_m << endl;
    cout << "k0g: " << k0g << " k1g: " << k1g << " k2g: " << k2g << " k3g: " << k3g << " k0m: " << k0m << endl;
    cout << "kbar0g: " << kbar0g << " kbar1g: " << kbar1g << " kbar2g: " << kbar2g << " kbar3g: " << kbar3g << " kbar0m: " << kbar0m << endl;
    ...
    vector<Reggeon> reggeons_gluon = computeReggeons(gluon, 0, 4);
    vector<Reggeon> reggeons_meson = computeReggeons(meson, 0, 1);
    vector<Reggeon> reggeons = {reggeons_gluon[0], reggeons_gluon[1], reggeons_gluon[2], reggeons_gluon[3], reggeons_meson[0]};
              ...
              &sigma_gg_IzNs, &sigma_gp_IzNs, &sigma_pp_IzNs, &reggeons] (const std::vector<double> & X)
    {
        ...
        vector<double> ks = {X[0], X[1], X[2], X[3], X[4]};
        vector<double> kbars = {X[5], X[6], X[7], X[8], X[9]};
        cout << "kg0: " << ks[0] << " kg1: " << ks[1] << " kg2: " << ks[2] << " kg3: " << ks[3] << " km0: " << ks[4] << endl;
        cout << "kbarg0: " << kbars[0] << " kbarg1: " << kbars[1] << " kbarg2: " << kbars[2] << " kbarg3: " << kbars[3]  << " kbarm0: " << kbars[4] << endl;
        /*
    ...
    // Start the fit now
    vector<double> X_guess = {k0g, k1g, k2g, k3g, k0m, kbar0g, kbar1g, kbar2g, kbar3g, kbar0m};
