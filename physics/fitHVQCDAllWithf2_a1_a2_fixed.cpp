...
double J(const vector<double> X)
{
    ...
    double a1 = 0, a2 = 1, xf = 2.0/3, tau0, Za, ca;
    ...
    try
    {
        ...
        TGMasses = computeMasses(zs, V2G, 2, "cheb");
    ...
    if (TGMasses.size() == 0) TGMasses = vector<double>(2,0);
    erms += fabs((TGMasses[0]/VMMasses[0] - 1.64)/1.64);
    erms += fabs((TGMasses[1]/VMMasses[0] - RTG_rho[0])/RTG_rho[0]);
